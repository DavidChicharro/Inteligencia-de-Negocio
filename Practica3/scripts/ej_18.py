...
'''
Selección de características e instancias
'''
# Eliminación de algunos outlayers
bd_no_outly = building_damage_cleaned[building_damage_cleaned['age'] <= 250]
bd_no_outly = bd_no_outly[bd_no_outly['area_percentage'] <= 80]
bd_no_outly = bd_no_outly[bd_no_outly['height_percentage'] <= 25]
bd_no_outly = bd_no_outly[bd_no_outly['count_families'] <= 4]

# Selección de características
building_damage_2 = bd_no_outly.drop(columns ="plan_configuration")
building_damage_2 = building_damage_2.drop(columns ="legal_ownership_status")
building_damage_2 = building_damage_2.drop(columns ="position")

# Quedan fuera 'plan_configuration' y 'legal_ownership_status' y 'position'
cat_feats = ['land_surface_condition', 'foundation_type', 'roof_type',
       'ground_floor_type', 'other_floor_type']

# Se convierten las variables categóricas en numéricas
train_final = pd.get_dummies(building_damage_2,columns=cat_feats,drop_first=True)
...
print("------ XGB...")
clf = xgb.XGBClassifier(max_depth=15, n_estimators = 2000, learning_rate=0.05, n_jobs=-1)
...
test_final.drop(labels=['position'], axis=1,inplace = True)
...
df_submission.to_csv("../submissions/submission_18.csv", index=False)
